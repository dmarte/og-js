...
export default class OgAuth extends OgQueryBuilder {
   ...
   *
   * @param {OgApi} api
   * @param {String} URL_PATH_LOGIN
   * @param {String} URL_PATH_USER
   * @param {String} KEY_SESSION_USER
   * @param {String} KEY_SESSION_TOKEN
   * @param {OgUserResource} USER_RESOURCE
   */
  constructor(
    api,
    {
  ...
  setTokenFromSession() {
    if (this.$session.has(this.KEY_TOKEN)) {
      this.token(this.$session.get(this.KEY_TOKEN));
    }
    return this;
  }
  ...
  setUserFromSession() {
    if (this.$session.has(this.KEY_USER)) {
      this.$user = new this.USER_RESOURCE(
        this.$api,
        this.$session.get(this.KEY_USER)
      );
    }
    return this;
  }
